...
async function handleUserBugReport(request, env, corsHeaders) {
  try {
    ...
    const body = [
      ...
      `| **Auto-Pick Verified** | ${settings.autoPickQuiz !== false ? 'Enabled' : 'Disabled'} |`,
      `| **Auto-Next when Answered** | ${settings.autoNextVerified !== false ? 'Enabled' : 'Disabled'} |`,
      `| **Auto-Next on Manual** | ${settings.autoNextQuiz ? 'Enabled' : 'Disabled'} |`,
      `| **Highlight Answers** | ${settings.autoHighlightQuiz !== false ? 'Enabled' : 'Disabled'} |`,
      `| **Smart Skip Answered** | ${settings.smartSkipQuiz ? 'Enabled' : 'Disabled'} |`,
      `| **Gemini AI Solver** | ${settings.aiQuizEnabled ? 'Enabled' : 'Disabled'} (Auto-Select: ${settings.aiAutoSelect ? 'Yes' : 'No'}) |`,
